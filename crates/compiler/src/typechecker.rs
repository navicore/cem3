...
impl TypeChecker {
    ...
    fn infer_statements_from(
        ...
        for (i, stmt) in statements.iter().enumerate() {
            // Skip this statement if we already handled it (e.g., pick/roll/times after literal)
            if skip_count > 0 {
                skip_count -= 1;
                continue;
            }

            // Pattern: [body] n times - allow non-stack-neutral quotations when n is literal
            if let Statement::Quotation { body, id, .. } = stmt
                && let Some(Statement::IntLiteral(n)) = statements.get(i + 1)
                && let Some(Statement::WordCall { name, .. }) = statements.get(i + 2)
                && name == "times"
            {
                let (new_stack, subst, effects) =
                    self.handle_literal_times(*id, body, *n, current_stack.clone())?;
                ...
                skip_count = 2; // Skip IntLiteral and "times"
                continue;
            }
    ...
    /// Handle `[body] n times` where n is a literal integer
    ///
    /// When the iteration count is known at compile time, we can type-check
    /// quotations with any stack effect by simulating n applications.
    ///
    /// Example: `[i.*] 4 times` with effect [Int Int -- Int]
    /// - Composed effect for 4 iterations: [Int Int Int Int Int -- Int]
    fn handle_literal_times(
        &self,
        quot_id: usize,
        body: &[Statement],
        n: i64,
        ...
        // Infer the quotation body's effect using infer_statements
        // This properly normalizes the effect by applying substitutions
        let body_effect = self.infer_statements(body)?;

        // Register quotation type for codegen
        let quot_type = Type::Quotation(Box::new(body_effect.clone()));
        self.quotation_types.borrow_mut().insert(quot_id, quot_type);

        if n <= 0 {
            // n=0 or negative: quotation never executes, stack unchanged
            return Ok((current_stack, Subst::empty(), vec![]));
        }

        // Apply the effect n times
        let mut stack = current_stack;
        let mut total_subst = Subst::empty();
        let mut all_effects: Vec<SideEffect> = Vec::new();

        for i in 0..n {
            let fresh_effect = self.freshen_effect(&body_effect);

            let (new_stack, iter_subst) =
                self.apply_effect(&fresh_effect, stack, &format!("times iteration {}", i + 1))?;

            stack = new_stack;
            total_subst = total_subst.compose(&iter_subst);

            // Accumulate side effects
            for effect in &fresh_effect.effects {
                if !all_effects.contains(effect) {
                    all_effects.push(effect.clone());
                }
            }
        }

        Ok((stack, total_subst, all_effects))
    }
...
mod tests {
    ...
    #[test]
    fn test_literal_times_non_stack_neutral() {
        // [i.*] 2 times on stack with 3 Ints should work
        ...
        assert!(
            result.is_ok(),
            "Literal times with non-stack-neutral quotation should pass: {:?}",
            result.err()
        );
    }

    #[test]
    fn test_literal_times_drop() {
        // [drop] 3 times should consume 3 values
        // Effect: [T T T -- ]
        let program = Program {
            includes: vec![],
            unions: vec![],
        ...
        assert!(
            result.is_ok(),
            "Literal times with drop should consume 3 values: {:?}",
            result.err()
        );
    }

    #[test]
    fn test_literal_times_dup() {
        // [dup] 3 times should produce extra values
        // Effect: [T -- T T T T]
        let program = Program {
            includes: vec![],
            unions: vec![],
        ...
        assert!(
            result.is_ok(),
            "Literal times with dup should produce extra values: {:?}",
            result.err()
        );
    }

    #[test]
    fn test_literal_times_zero() {
        // [anything] 0 times should be identity
        let program = Program {
            includes: vec![],
            unions: vec![],
            words: vec![WordDef {
        ...
        assert!(
            result.is_ok(),
            "Literal times with 0 should be identity: {:?}",
            result.err()
        );
    }

    #[test]
    fn test_literal_times_underflow_error() {
        // [i.*] 5 times on stack with only 2 Ints should fail
        // with clear error about which iteration failed
        let program = Program {
            includes: vec![],
            unions: vec![],
            words: vec![WordDef {
        ...
        assert!(
            result.is_err(),
            "Literal times with stack underflow should fail"
        );
        // Verify error message mentions which iteration failed
        let err = result.unwrap_err();
        assert!(
            err.contains("iteration"),
            "Error should mention iteration number: {}",
            err
        );
    }

    #[test]
    fn test_literal_times_type_mismatch_error() {
        // [i.*] expects Ints, but we provide Strings - should fail
        let program = Program {
            includes: vec![],
            unions: vec![],
            words: vec![WordDef {
        ...
        assert!(
            result.is_err(),
            "Literal times with type mismatch should fail at compile time"
        );
    }

    #[test]
    fn test_literal_times_output_mismatch_error() {
        // [i.*] 2 times on 3 Ints produces 1 Int, but we declare output as 2 Ints
        let program = Program {
            includes: vec![],
            unions: vec![],
            words: vec![WordDef {
        ...
        assert!(
            result.is_err(),
            "Literal times with wrong output declaration should fail"
        );
    }
}
